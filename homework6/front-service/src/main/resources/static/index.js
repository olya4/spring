...
//          $http.get('http://localhost:8189/winter-app/api/v1/products')
...
//          };
    ...
    $scope.loadProducts = function (pageIndex = 1) {
        $http({
            url: 'http://localhost:5555/winter-app/api/v1/products',
            method: 'GET',
            params: {
                // переменная filter собрана из полей формы в store.html
   ...
   $scope.loadCart = function(){
        $http.get('http://localhost:5555/winter-carts/api/v1/cart')
           .then(function successCallback(response) {
        // в теле ответа будет json-объект корзинки
        // состоит из списка продуктов (items) - массив строк корзинки
        // и суммарной стоимости (totalPrice)
   ...
   $scope.addToCart = function(productId){
        $http.get('http://localhost:5555/winter-carts/api/v1/cart/add/' + productId)
          .then(function successCallback(response) {
            $scope.loadCart();
         });
   }
   ...
   $scope.removeFromCart = function(productId){
        $http.get('http://localhost:5555/winter-carts/api/v1/cart/remove/' + productId)
          .then(function successCallback(response) {
            $scope.loadCart();
         });
    }
   ...
   $scope.clearCart = function(){
            $http.get('http://localhost:5555/winter-carts/api/v1/cart/clear')
              .then(function successCallback(response) {
                $scope.loadCart();
             });
   }
   ...
   $scope.createOrder = function(){
            $http.post('http://localhost:5555/winter-app/api/v1/orders')
              .then(function successCallback(response) {
     ...
     // $scope.user - два поля из index.html из формы авторизации (user.username, user.password)
         $http.post('http://localhost:5555/winter-auth/auth', $scope.user)
